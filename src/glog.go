...
type Log struct {
	...
	FlushTime time.Duration //millisecond
...
func New(args ...interface{}) (*Log, error) {
	...
	var (
		...
		//default Log
		log = Log{
			Buf:       DEFAULT_BUFFER_SIZE, //bytes
			FlushTime: 1000,                //millisecond
	...
	logFlushTicker = time.NewTicker(log.FlushTime * time.Millisecond)
	glog = &log
	return glog, err
}
